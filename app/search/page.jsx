...
export default function SearchPage() {
  ...
  useEffect(() => {
    const fetchArticles = async () => {
      ...
      try {
        setLoading(true)
        const url = `/api/news?searchQuery=${query}`
  ...
  const handleSearch = (e) => {
    e.preventDefault()
    window.history.pushState({}, "", `/search?q=${encodeURIComponent(inputQuery)}`)
    setSearchQuery(inputQuery)

    const fetchArticles = async () => {
      try {
        setLoading(true)
        const url = `/api/news?searchQuery=${inputQuery}`
        const response = await fetch(url)
        const data = await response.json()
        setArticles(data)
      } catch (error) {
  ...
  return (
    <div className="max-w-screen-xl mx-auto p-5">
      ...
      >
        <div className="fancy-card px-8 py-6 mx-6 bg-gradient-to-r from-blue-500 to-purple-500 w-full max-w-3xl">
          <h1 className="text-3xl sm:text-4xl text-center text-white font-bold mb-6">
          ...
          <form onSubmit={handleSearch} className="flex w-full">
            <input
              type="text"
              className="w-full p-4 rounded-l-lg border-0 focus:ring-2 focus:ring-blue-300 text-gray-800"
            ...
            <button
              type="submit"
              className="bg-white text-blue-600 px-6 py-4 rounded-r-lg font-medium hover:bg-blue-50 transition-colors duration-300"
            >
              Search
            </button>
          </form>
      ...
      {searchQuery && (
        ...
        >
          <p className="text-xl text-gray-700">
            {loading ? "Searching..." : `Found ${articles.length} results for "${searchQuery}"`}
          </p>
        </motion.div>
      )}
      ...
      {loading ? (
        ...
        >
          ...
          <p className="text-gray-500">
            {searchQuery
              ? `We couldn't find any articles matching "${searchQuery}". Please try different keywords.`
